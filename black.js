...
mod . exports  =  async  ( customer ,  message )  =>  {

        let  user  =  message . guild . members . get ( customer.user.id ) 
    ...
    customer . db . query ( `SELECT * FROM config WHERE serverID = ' ${ message.guildID } ' ` ,  async ( err , config ) => {   
        if ( ( config . length  <  1 )  ||  ( config [ 0 ] . antispam  ===  "no" ) )  { }  else  {
            if  ( ! client . spam . get ( message . author . id ) )  {
                customer . spam . set ( message.author.id , { _ _ _ _ 
                    _id : message . author . ID ,
                    msg : 1 ,
                } ) ;

                function  co ( )  {
                    customer . spam . remove ( message.author ) ; _ _
                }

                setTimeout ( co ,  10000 ) ;
            }  else  {
                let  mss  =  customer . spam . get ( message.author.id ) . _ _ _ _ msg + 1  
                customer . spam . set ( message.author.id , { _ _ _ _ 
            ...
            if  ( client.spam.get ( message.author.id ) .msg === config [ 0 ] .maxspam ) { _ _ _ _ _ _ _ _ _ _   
                ...
                try  {
                    ...
                    if  ( config [ 0 ] .securitylvl === "  low  " )  {
                        ...
                        await  message . channel . purge ( client.spam.get ( message.author.id ) .msg , filter ) _ _ _ _ _ _ _ _ _ 
